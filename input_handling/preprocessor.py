...
def preprocess_code(
    code: str, filename: Optional[str] = "submitted_snippet"
) -> PreprocessedCode:
    """Normalizes code line endings, computes line offset maps, and validates syntax with ast.parse.

    Args:
        code: Submitted raw source code string.
        filename: Optional filename or identifier for AST tracking.

    Returns:
        PreprocessedCode: Model containing normalized code, AST tree (if valid), and any syntax Issue.
    """
    ...
    # 3. Validate Python syntax with ast.parse (without executing code)
    ast_tree = None

...
class TestFullPipelineIntegration:
    ...
    def test_4_empty_input_handled_safely(self):
        """Test 4: Empty input is validated safely without exceptions."""
        result = review_code("")

        assert isinstance(result, ReviewResult)
        assert len(result.issues) == 0
        assert result.summary.total_issues == 0
        assert "empty" in result.score.summary_notes.lower()

    def test_5_oversized_input_rejected(self):
        """Test 5: Oversized input exceeding MAX_CODE_CHARS is rejected."""
        settings = get_settings()
        pad_count = (settings.MAX_CODE_CHARS // 5) + 100
        oversized_code = "# Padding\n" + ("x = 1\n" * pad_count)

        result = review_code(oversized_code)

        assert isinstance(result, ReviewResult)
        assert len(result.issues) == 0
        assert result.score.overall_score == 0.0
        assert "exceeds maximum limit" in result.score.summary_notes.lower()
